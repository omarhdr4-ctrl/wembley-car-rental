...
// Charges database operations
 const loadCharges = async () => {
  try {
    ...
    console.log('Charges loaded:', data);
    setCharges(data || []);
  ...
  try {
    ...
    console.log('Saving charge data:', chargeData);
    
    // Prepare the data with proper validation - REMOVE the id field completely
    const dataToInsert = {
      description: chargeData.description || '',
      date: chargeData.date || '',
      amount: parseFloat(chargeData.amount) || 0,
      car_id: chargeData.carId || null, // This can be null for general expenses
      car_name: chargeData.carName || null,
      plate_number: chargeData.plateNumber || null,
      remarks: chargeData.remarks || null,
      category: chargeData.category || 'maintenance'
    };
    
    // Validate required fields
    if (!dataToInsert.description.trim()) {
    ...
    if (error) {
      ...
      alert(`Error saving charge: ${error.message}`);
      return false;
    }
    
    console.log('Charge saved successfully:', data);
    return true;
  } catch (error) {
...
// Filter and availability handlers
  ...
  if (editingCharge) {
    success = await updateCharge(chargeFormData);
    if (success) {
      alert('Charge updated successfully!');
      setEditingCharge(null);
    }
  } else {
    success = await saveCharge(chargeFormData);
    if (success) {
  ...
  if (success) {
    await loadCharges(); // Reload the charges list
    setShowChargeForm(false);
    setChargeFormData({
      description: '', date: '', amount: 0, carId: '', carName: '', 
      plateNumber: '', remarks: '', category: 'maintenance'
    });
  }
};
...
if (!isLoggedIn) {
  return (
    <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#1e3a8a' }}>
      <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '10px', textAlign: 'center' }}>
        <h2>WEMBLEY CAR - Enter Password</h2>
        <input 
          type="password" 
